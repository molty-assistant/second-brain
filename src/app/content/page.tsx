import { getPipelineItems, type PipelineItem } from '@/lib/hub';
...
const typeColors: Record<PipelineItem['type'], string> = {
...
function Column({ title, items, icon: Icon, color }: ColumnProps) {
  return (
      ...
      <div className="space-y-2">
        {items.map((item) => (
          ...
          >
            ...
            {item.content && <p className="text-sm text-[#8b949e] line-clamp-2">{item.content}</p>}
...
export default function ContentPage() {
    ...
    <div className="min-h-screen bg-[#0d1117]">
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="mb-8">
          <Link
            href="/"
            className="inline-flex items-center gap-1 text-sm text-[#8b949e] hover:text-[#58a6ff] mb-4"
          >
          ...
          <p className="text-[#8b949e] mt-2">LinkedIn posts, articles, talks — from idea to published</p>
        </div>
        ...
        <div className="mt-8 bg-[#161b22] border border-[#30363d] rounded-lg p-4">
          ...
          <p className="text-sm text-[#8b949e] mb-2">
            ...
            <code className="bg-[#21262d] px-1 rounded">content/pipeline/</code>:
          </p>
 ...
 Notes, outline, draft content...`}</pre>
        </div>
      </div>
    </div>
  );
