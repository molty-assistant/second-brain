...
type TaskUpdates = Partial<
  Pick<Task, 'title' | 'status' | 'priority' | 'assignee' | 'workOrderId' | 'notes' | 'content' | 'completed'>
>;
...
export async function updateTask(id: string, updates: TaskUpdates) {
  ...
  if (
    ...
    normalized.workOrderId !== null &&
    String(normalized.workOrderId).trim() === ''
  ) {
    normalized.workOrderId = null;
  }
...
export async function updatePipelineStatus(slug: string, newStatus: string) {
  ...
  savePipelineItem({ ...item, status: newStatus as 'idea' | 'drafting' | 'review' | 'published' });
  revalidatePath('/content');
  revalidatePath('/');
  return { success: true };
}
